...
import { USER_TYPES } from '../utils/index.js'
...
export const webhook = async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        if (user) {
            ...
            user.user_type = USER_TYPES.PREMIUM
            await user.save()
...
export const createPackage = async ({ name, description, amount, interval = 'month', currency = 'usd', features }) => {
    ...
    const product = await stripe.products.create({
        name,
        description,
        marketing_features: features.map(item => ({ name: item }))
    })

    const is_one_time = interval === 'forever'

    const price = await stripe.prices.create({
        unit_amount,
        currency,
        ...(is_one_time ? {} : { recurring: { interval } }),
        product: product.id
    })
    ...
    const data = {
        ...
        interval: is_one_time ? 'forever' : price.recurring.interval,
...
export const getAllPackages = async (filters = {}) => {
    ...
    const packages = prices.data.sort(
        ...
        return {
            ...
            description: product.description,
            amount: price.unit_amount / 100,
            interval: price.recurring ? price.recurring.interval : 'forever',
            currency: price.currency,
            features: product.marketing_features.map(item => item.name)
        }
...
export const subscribe = async (id, user_id) => {
    ...
    const price = await getPackageDetails(id)
    const is_one_time = price.interval === 'forever'
...
export const getActiveSubscription = async (id) => {
    ...
    return {
        ...
        interval: active_subscription.items.data[0].price.recurring ? active_subscription.items.data[0].price.recurring.interval : 'forever',
